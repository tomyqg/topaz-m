...
void AD_UpdatePdReadData( void* pxPdDataBuf )
{
   ...
   if( AD_paiPdReadMap )
   {
      for ( i = 0; i < ad_ReadMapInfo.iNumMappedAdi; i++ )
      {
         ...
         {
            if( AD_paiPdReadMap->iAdiIndex >= ad_iNumOfADIs )
            {
               /*
               ** Pull the plug! The data in these tables should already have
               ** been checked and should be OK!
               */
               ABCC_ERROR( ABCC_SEV_FATAL, ABCC_EC_ERROR_IN_READ_MAP_CONFIG, (UINT32)(AD_paiPdReadMap->iAdiIndex) );
            }
...
BOOL AD_UpdatePdWriteData( void* pxPdDataBuf )
{
   ...
   if( paiPdWriteMap )
   {
      ...
      for( i = 0; i < ad_WriteMapInfo.iNumMappedAdi ; i++)
      {
         ...
         {
            if( paiPdWriteMap->iAdiIndex >= ad_iNumOfADIs )
            {
               /*
               ** Pull the plug! The data in these tables should already have
               ** been checked and should be OK!
               */
               ABCC_ERROR( ABCC_SEV_FATAL, ABCC_EC_ERROR_IN_WRITE_MAP_CONFIG, (UINT32)(paiPdWriteMap->iAdiIndex) );
            }
